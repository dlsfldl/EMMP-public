...
def check_entrance_traj(entrances_batch, trajs_batch):
    ...
    entrance1_batch = entrances_batch[0].transpose(1, 2)
    entrance2_batch = entrances_batch[1].transpose(1, 2)
    
    p_entrance1 = entrance1_batch[:, :1]
    q_entrance1 = entrance1_batch[:, 1:]
    p_entrance2 = entrance2_batch[:, :1]
    q_entrance2 = entrance2_batch[:, 1:]
    
    p_entrance = torch.cat([p_entrance1, p_entrance2], dim=1)
    q_entrance = torch.cat([q_entrance1, q_entrance2], dim=1)
    
    p_entrance_repeat = p_entrance.unsqueeze(2).repeat(1, 1, trajs_batch.shape[1]- 1, 1).reshape(-1, 2)
    q_entrance_repeat = q_entrance.unsqueeze(2).repeat(1, 1, trajs_batch.shape[1]- 1, 1).reshape(-1, 2)
    
    p_traj = trajs_batch[:, :-1]
    p_traj_repeat_flat = p_traj.repeat(1, p_entrance.shape[1], 1).reshape(-1, 2)
    q_traj = trajs_batch[:, 1:]
    q_traj_repeat_flat = q_traj.repeat(1, p_entrance.shape[1], 1).reshape(-1, 2)
    
    intersect = intersection_check_batch(
    p_entrance_repeat, q_entrance_repeat,
...
def collision_check_wall_traj(walls_batch, trajs_batch):
    if trajs_batch.device is not torch.device('cpu'):
        trajs_batch = trajs_batch.cpu()
    batch_size = len(trajs_batch)
    trajs_batch = trajs_batch.reshape(batch_size, -1, 2)
